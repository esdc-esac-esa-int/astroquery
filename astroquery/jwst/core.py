...
class JwstClass(object):
    ...
    def __init__(self, tap_plus_handler=None, data_handler=None):
        if tap_plus_handler is None:
            self.__jwsttap = TapPlus(url="http://jwstdev.esac.esa.int/"
                                     "server/tap", data_context='data')
        else:
            self.__jwsttap = tap_plus_handler

        if data_handler is None:
            self.__jwstdata = JwstDataHandler(base_url="http://jwstdev"
                                              ".esac.esa.int/server/data?")
    ...
    def get_related_observations(self, observation_id):
        ...
        if any(job.get_results()["observationid"]):
            ...
            query_members = "select m.members from {} m where m.observationid"\
                      "='{}'".format(self.JWST_MAIN_TABLE, observation_id)
            job = self.__jwsttap.launch_job(query=query_members)
            oids = job.get_results()["members"][0].\
    ...
    def _query_get_product(self, artifact_id=None, file_name=None):
        if(file_name):
            ...
            return job.get_results()['artifactid'][0]
        else:
            ...
            return job.get_results()['filename'][0]
