...
class JwstClass(object):
    ...
    def __init__(self, tap_plus_handler=None, data_handler=None):
        if tap_plus_handler is None:
            self.__jwsttap = TapPlus(url="http://jwstdev.esac.esa.int/server/tap", data_context='data')
        else:
    ...
    def get_product_list(self, observation_id=None,
        ...
        cal_level : str, optional
            Calibration level. Possible values: 'ALL', 'SELECTED', '1'
        ...
        if observation_id is None:
            raise ValueError("'observation_id is a mandatory requirement'")

        plane_id, max_cal_level = self.__get_plane_id(observation_id)
        
        if (cal_level == 3 and cal_level>max_cal_level):
            raise ValueError('LOOK FOR TOP');

        list = self.__get_associated_planes(plane_id, cal_level, False)
    ...
    def __get_associated_planes(self, plane_id, cal_level, is_url):
        if (cal_level == 'SELECTED'):
            if (not is_url):
            ...
            if (not is_url):
                list = "('{}')".format("', '".join(
                    plane_id_table['product_planeid'].pformat(show_name=False)))
            else:
                list = "{}".format(",".join(
                    plane_id_table['product_planeid'].pformat(show_name=False)))
        return list
    ...
    def __get_sibling_planes(self, planeid, cal_level='ALL'):
        where_clause = ''
        if (cal_level == 'ALL'):
            where_clause = 'WHERE sp.calibrationlevel<=p.calibrationlevel AND p.planeid =';
        else:
            where_clause = 'WHERE sp.calibrationlevel={} AND p.planeid ='.format(cal_level);
        try:
            ...
            log.info('sibling ' + query_siblings)
            return job.get_results()
        except Exception as e:
            raise ValueError(e)
    ...
    def __get_member_planes(self, planeid, cal_level='ALL'):
        ...
        if (cal_level == 'ALL'):
            ...
            where_clause = 'WHERE mp.calibrationlevel={} AND p.planeid ='.format(cal_level);
        try:
            ...
            log.info('members ' + query_members)
            return job.get_results()
        except Exception as e:
            raise ValueError(e)
    ...
    def get_obs_products(self, observation_id=None, cal_level=None, product_type=None, output_file=None):
        ...
        if (cal_level == 2 and max_cal_level != 2):
            plane_id = self.__get_associated_planes(plane_id, 2, True)

        params_dict['planeid'] = plane_id
        self.__set_additional_parameters(params_dict, cal_level, max_cal_level, product_type)
    ...
    def __set_additional_parameters(self, param_dict, cal_level, max_cal_level, product_type):
