...
class Conf(_config.ConfigNamespace):
    """
    Configuration parameters for `astroquery.pla`.
    """
    DATA_ACTION = _config.ConfigItem("http://archives.esac.esa.int/" +
                                     "epla-sl-server/servlet/data-action?",
                                     "Main url for retriving pla files")
    METADATA_ACTION = _config.ConfigItem("http://archives.esac.esa.int/" +
                                         "epla-sl-server/servlet/" +
                                         "metadata-action?",
                                         "Main url for retriving pla metadata")

conf = Conf()

__all__ = ['Pla', 'PlaClass', 'Conf', 'conf', 'PlaHandler', 'Handler']


class PlaHandler(object):
...
class PlaClass(object):
    ...
    def __init__(self, url_handler=None, tap_handler=None):
        ...
        if tap_handler is None:
            self.__tap = TapPlus(url="http://pla.esac.esa.int"
                                 "/tap-server/tap/")
        else:
    ...
    def query_pla_tap(self, query, output_file=None,
